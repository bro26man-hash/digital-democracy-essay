...
### F. Off-Chain Signatures (Snapshot-Style)
...
The TerraBioDAO contract's deposit-and-lock mechanism is a partial mitigation: by requiring tokens to be locked for a period, it makes flash-loan attacks more difficult because the loan must remain deposited for the lock duration. The DA0-DA0 cw-vesting module serves a similar function for its governance tokens.
...
1. **Zero-Knowledge Proofs for Voting** — ZK-snarks and ZK-starks could enable fully private yet fully verifiable on-chain voting, where a voter can prove they voted correctly without revealing their choice. This is the most promising path to resolving the privacy/transparency tension. The Skrynka design's use of homomorphic authenticators (Shacham-Waters) to shrink audit bandwidth ~50× is a related primitive that could enable efficient private voting.

2. **Identity & Sybil Resistance** — Democratic governance requires one-person-one-vote. Blockchain voting struggles with Sybil attacks (one person creating many wallets). Solutions range from proof-of-personhood (Worldcoin, Idena) to social verification graphs to the git-commit-based identity of gov4git. The Skrynka paper's quorum-held key scheme demonstrates that Sybil resistance can be achieved through *cryptographic custody* rather than *identity verification* — a paradigm shift worth exploring for DAO governance.
...
- [Handshake Web Issue #14 — Community-Operated Marketing Governance](https://github.com/handshake-org/handshake-web/issues/14)
